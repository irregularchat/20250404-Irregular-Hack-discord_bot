...
class EmailHandler:
    def __init__(self):
        self.server = None
        self.last_checked = datetime.now() - timedelta(hours=1)  # Start by checking emails from the last hour
    
    def connect(self):
        """Connect to the IMAP server"""
        try:
            logger.info(f"Connecting to IMAP server: {config.IMAP_SERVER}")
            self.server = IMAPClient(config.IMAP_SERVER, port=config.IMAP_PORT, use_uid=True, ssl=True)
    ...
    def get_new_emails(self):
        ...
        try:
            ...
            # Fetch email data
            email_data = []
            for uid, message_data in self.server.fetch(messages, ['ENVELOPE', 'RFC822']).items():
                try:
                    ...
                    if config.WHITELISTED_EMAIL_ADDRESSES and not any(addr in from_address for addr in config.WHITELISTED_EMAIL_ADDRESSES):
                        logger.info(f"Skipping email from non-whitelisted address: {from_address}")
                        continue

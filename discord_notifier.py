...
DATE_PATTERN = re.compile(
    r"\b(Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:tember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)\s+\d{1,2}(?:st|nd|rd|th)?(?:,?\s+\d{4})?\b|\b\d{1,2}(?:st|nd|rd|th)?\s+(?:of\s+)?(Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:tember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)(?:,?\s+\d{4})?\b|\b(?:tomorrow|next week|next month)\b",
    re.IGNORECASE,
)
...
class DiscordNotifier(commands.Bot):
    ...
    async def on_ready(self):
        ...
        if not channel:
            logger.error(
                f"Could not find channel with ID {self.notification_channel_id}"
            )
            return
    ...
    async def send_email_notification(self, email_data):
        ...
        try:
            ...
            embed.add_field(
                name="Email Length", value=f"{body_length} characters", inline=True
            )
            ...
            if summary:
                ...
                dates = DATE_PATTERN.findall(formatted_summary)
                for date in dates:
            ...
            if body:
                ...
                truncated_body = (
                    body[:max_preview_length] + "..."
                    if body_length > max_preview_length
                    else body
                )
                embed.add_field(
                    name="📄 Content Preview",
                    value=f"```{truncated_body}```",
                    inline=False,
                )
                ...
                if body_length > max_preview_length:
                    embed.set_footer(
                        text=f"Full email: {body_length} characters ({round(body_length/max_preview_length, 1)}x longer than preview)"
                    )

            # Send the embed to the channel
            await channel.send(embed=embed)
            logger.info(
                f"Sent notification for email: {subject} - {body_length} characters"
            )
            return True
